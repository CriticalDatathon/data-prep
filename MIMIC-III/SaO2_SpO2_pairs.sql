...
DROP TABLE IF EXISTS `golden-rite-376204.mimiciii_pulseOx.SaO2_SpO2_pairs`;
CREATE TABLE `golden-rite-376204.mimiciii_pulseOx.SaO2_SpO2_pairs` AS
...
WITH pairs AS (
  ...
  LEFT JOIN(
    ...
    FROM `physionet-data.mimiciii_clinical.chartevents` spo2_table
    WHERE itemid = 220277 -- SpO2
  )
  AS spo2_table
  ON spo2_table.icustay_id = sao2_table.icustay_id
...
AND spo2_vals.icustay_id = pairs.icustay_id
